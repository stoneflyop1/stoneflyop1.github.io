...
一般地，“随机样本”表示各个样本点具有相同的概率。有放回的样本概率为 $n^{_r}$，无放回的概率则为 $1/(n)_r$。若n远远大于r，则两个值接近相等。
...
$$ \left( \begin{array}{c} n \\ r \end{array} \right) \mbox{~=~} \left( \begin{array}{c} n \\ (n-r) \end{array} \right) $$
...
我们有：${q_1}^*+{q_2}^*+...=1$，因此无需引入一个样本点表示从来没有一个球放入给定的盒a中。过程在第r步以后才结束的概率：
